...
class CLI
{
    ...
    private function createController($name)
    {
        ...
        if (file_exists($path)) {
            $this->printLine("\033[1;33mO controller {$name}Controller already exists.\033[0m");
        } else {
            file_put_contents($path, $template);
            $this->printLine("\033[1;32mController {$name}Controller successfully created in {$path}.\033[0m");
        }
    }
    ...
    private function createModel($name)
    {
        ...
        if (file_exists($path)) {
            $this->printLine("\033[1;33mO model {$name}Model already exists.\033[0m");
        } else {
            file_put_contents($path, $template);
            $this->printLine("\033[1;32mModel {$name}Model successfully created in {$path}.\033[0m");
        }
    }
